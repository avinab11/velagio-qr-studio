...
const GoPage: React.FC = () => {
  ...
  // Build the WIFI: URI that iOS/Android use to pre-fill WiFi join dialog
  const getWifiUri = (info: WiFiInfo) => {
    return `WIFI:S:${info.ssid};T:${info.type};P:${info.password};H:${info.hidden ? 'true' : 'false'};;`;
  };
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        {pageType === 'wifi' && wifiInfo && (
          <div className="apple-card p-8 space-y-6">
            ...
            >
              <a href={getWifiUri(wifiInfo)}>
                <Wifi className="w-5 h-5 mr-2" /> Join The Network
              </a>
            </Button>
            ...
            {wifiInfo.password && (
              <button
                ...
                className="w-full text-center text-xs text-primary font-medium py-1 hover:underline transition-all"
              >
                {copied ? 'Password copied!' : 'Or copy password manually'}
              </button>
            )}
